...
export const TodoCard: React.FC<TodoCardProps> = ({ todo, dependentTodo, filter, onEdit, onDelete }) => {
  const now = new Date();
  ...
  return (
    <div className={`${cardBgClass} rounded-lg shadow-md p-4 border flex flex-col justify-between transition-shadow hover:shadow-lg`}>
      <div>
      ...
      <div>
        ...
        <div className="flex justify-end space-x-2 mt-4">
          <button onClick={() => onEdit(todo.id)} className="text-sm bg-slate-500 hover:bg-slate-600 text-white font-semibold py-1 px-3 rounded-md">編集</button>
          <button onClick={() => onDelete(todo.id)} className="text-sm bg-red-500 hover:bg-red-600 text-white font-semibold py-1 px-3 rounded-md">削除</button>
        </div>
      </div>
    </div>
  );
